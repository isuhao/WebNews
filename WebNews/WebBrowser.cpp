...
BOOL CAsyncWebBrowser::Create( HWND hParent, const CRect& rcClient )
{
    ...
    Wait( m_hEventThreadRun );
    
    return ( NULL != m_hThread );
}

void CAsyncWebBrowser::Destroy()
{
    ::SetEvent( m_hEventThreadRun );
    ::CloseHandle( m_hEventThreadRun );
    m_hEventThreadRun = NULL;
...
UINT CAsyncWebBrowser::WebBrowserThread()
{
    ...
    for ( ;; )
    {
        ...
        if ( msg.message == WM_WEBBROWSER_METHORD )
        {
            __try
            {
                if ( S_FALSE == OnWebbrowserMethord( msg.wParam, msg.lParam ) )
                {
                    break;
                }
            }
...
BOOL CAsyncWebBrowser::Wait( HANDLE hThread )
{
    DWORD dwRet = 0;
    MSG msg;
    DWORD bRet = 0;
